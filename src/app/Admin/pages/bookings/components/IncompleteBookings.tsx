...
export default function IncompleteBookings({
  ...
  // Only not-booked bookings, ordered by saved time (newest first)
  const incompleteSorted = useMemo(
    () =>
      data
        .filter((b) => !b.isBooked)
        .slice()
        .sort((a, b) => parseCreatedAt((b as any).createdAt) - parseCreatedAt((a as any).createdAt)),
    [data]
  );

  const openModal = (b: BookingData) => {
    setSelected(b);
    const initial = ((b as any).reedStatus as Status | undefined) ?? 'read';
    setStatus(initial);
    setOpen(true);
  };
  ...
  async function handleSaveStatus() {
    ...
    try {
      ...
      setData((prev) => prev.map((b) => (b.id === selected.id ? ({ ...b, reedStatus: status } as any) : b)));
      setSelected((prev) => (prev ? ({ ...prev, reedStatus: status } as any) : prev));
    } catch (e) {
      console.error(e);
        ...
        <table className="min-w-full text-sm">
          ...
          <tbody>
            {incompleteSorted.map((booking, idx) => {
              const s: Status = ((booking as any).reedStatus as Status) ?? 'read';
              const color =
          ...
          <div className="relative w-full max-w-2xl rounded-2xl bg-white shadow-2xl">
            ...
            <div className="max-h-[70vh] overflow-y-auto px-5 py-4">
              <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
                ...
                <Detail label="Pickup Date" value={selected.pickupDate} />
                <Detail label="Pickup Time" value={(selected as any).pickupTime} />
                <Detail label="Return Location" value={selected.returnLoc} />
                <Detail label="Return Date" value={selected.returnDate} />
                <Detail label="Return Time" value={(selected as any).returnTime} />
                <Detail label="Saved At" value={formatCreatedAt((selected as any).createdAt)} />
                {selected.id && <Detail label="ID" value={selected.id} />}
              </div>

              {/* Status row (control + save button on the same line) */}
              <div className="mt-4 flex flex-wrap items-center gap-3">
                ...
                <div className="text-xs text-gray-500">
                  Current:{' '}
                  <span className="font-medium">{(((selected as any).reedStatus as Status) ?? 'read')}</span>
                </div>
              </div>
            </div>
